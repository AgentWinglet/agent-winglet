...
EOF
  ...
  ############################################################################
  # macOS tray helper (menu-bar glance icon + login item — see
  # cmd/agent-winglet-tray's doc comment). `make app` above already built
  # and nested it inside the just-installed Winglet.app (see the Makefile's
  # nest-tray-darwin target); this script only asks macOS to register it.
  ############################################################################
  if [ "$OS" = "darwin" ]; then
    stop_tray
